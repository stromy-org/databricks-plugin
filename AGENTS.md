...
databricks is a Claude Code plugin for Databricks. It is a **distribution artifact** — skills are authored in workspace-studio and cherry-picked here for client deployment.
...
├── companies/databricks/  # Brand data (charter, logos, colors)
...
This plugin defaults to **Databricks** brand data at `companies/databricks/`. Skills read `charter.json`, `logos/`, `images/`, and `tokens.css` from this path automatically — no prompt-time client selection needed.

For collaborative brands (e.g., "Databricks x Partner"), skills fall back to the Databricks charter for any missing fields.

**Slug discipline (`client_slug` is `databricks`):**
...
- Default brand: `companies/databricks/` — hardwired, no discovery needed
